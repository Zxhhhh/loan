...
public class PersonalLoanTask {
	...
	public void checkLoanStatus(){
		...
		List<PersonalLoanInfo> loans  = personalLoanService.getPersonalLoanInfoByIsRepay((byte)0);
		
		Long nowTime = new Date().getTime();
		
		//遍历所有个人贷款
		for(PersonalLoanInfo loan:loans){
			//转化为毫秒:1s = 1000ms
			...
			if(nowTime>deadLineTime){//个人贷款超时还款
				...
				Integer count = personalLoanInfoMapper.updateByPrimaryKey(loan);
				if(count!=0){
					/*增加一条消息给用户
					 * 1.数据库增加
					 * 2.即时推送一波
					...
					String content  = "您有一笔截止于:"+deadlineStr+"的个人贷款超时还款，请及时与管理员联系";
				...
				String content  = "您有一笔截止于:"+deadlineStr+"的个人贷款即将到期，距离截至还有: "+repayTimeStr;
				String userId = loan.getUserId();
				Remind remind = new Remind((byte)1,(byte)0,userId,content);//状态未读，消息类型为【贷款反馈】
				LoanResult<Remind> remindResult = remindService.addRemind(remind);
				if(remindResult.isSuccess){
